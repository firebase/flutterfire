  ...
  private fun startAt(modifier: Map<String, Any?>) {
    ...
    query =
        when (value) {
          ...
          is Number ->
              if (key == null) query.startAt(value.toDouble())
              else query.startAt(value.toDouble(), key)
          else ->
              if (key == null) query.startAt(value.toString())
              else query.startAt(value.toString(), key)
        }
  }
  ...
  private fun startAfter(modifier: Map<String, Any?>) {
    ...
    query =
        when (value) {
          ...
          is Number ->
              if (key == null) query.startAfter(value.toDouble())
              else query.startAfter(value.toDouble(), key)
          else ->
              if (key == null) query.startAfter(value.toString())
              else query.startAfter(value.toString(), key)
        }
  }
  ...
  private fun endAt(modifier: Map<String, Any?>) {
    ...
    query =
        when (value) {
          ...
          is Number ->
              if (key == null) query.endAt(value.toDouble()) else query.endAt(value.toDouble(), key)
          else ->
              if (key == null) query.endAt(value.toString()) else query.endAt(value.toString(), key)
        }
  }
  ...
  private fun endBefore(modifier: Map<String, Any?>) {
    ...
    query =
        when (value) {
          ...
          is Number ->
              if (key == null) query.endBefore(value.toDouble())
              else query.endBefore(value.toDouble(), key)
          else ->
              if (key == null) query.endBefore(value.toString())
              else query.endBefore(value.toString(), key)
        }
  }
}
