      ...
      val (code, message) =
          when (errorCode) {
            DatabaseError.DATA_STALE ->
                "data-stale" to "The transaction needs to be run again with current data."
            DatabaseError.OPERATION_FAILED ->
                "failure" to "The server indicated that this operation failed."
            DatabaseError.PERMISSION_DENIED ->
                "permission-denied" to "Client doesn't have permission to access the desired data."
            DatabaseError.DISCONNECTED ->
                "disconnected" to "The operation had to be aborted due to a network disconnect."
            ...
            DatabaseError.OVERRIDDEN_BY_SET ->
                "overridden-by-set" to "The transaction was overridden by a subsequent set."
            DatabaseError.UNAVAILABLE -> "unavailable" to "The service is unavailable."
            DatabaseError.NETWORK_ERROR ->
                "network-error" to "The operation could not be performed due to a network error."
            DatabaseError.WRITE_CANCELED ->
                "write-cancelled" to "The write was canceled by the user."
    ...
    fun unknown(errorMessage: String?): FlutterFirebaseDatabaseException {
      ...
      when {
        ...
        message?.contains("Permission denied") == true ||
            message?.contains("Client doesn't have permission") == true -> {
