...
@objc class FLTFirebaseDatabaseHostApi: NSObject, FirebaseDatabaseHostApi {
  ...
  private func createFlutterError(_ error: Error) -> PigeonError {
    ...
    let code = parts[0]
    let message = parts[1]
    // Provide details so Dart can preserve the code/message in FirebaseException
  ...
  func queryGet(app: DatabasePigeonFirebaseApp, request: QueryRequest,
    ...
    // Apply query modifiers (same logic as queryObserve) with handling for missing order
    var query: DatabaseQuery = reference
    var hasOrderModifier = false
    for modifier in request.modifiers {
      guard let type = modifier["type"] as? String else { continue }
