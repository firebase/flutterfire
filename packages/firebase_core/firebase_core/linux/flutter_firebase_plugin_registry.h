...
class FlutterFirebasePluginRegistry {
 public:
  ...
  // constants map.
  //
  // Ownership: returns a new FlValue* map with a reference owned by the
  // caller (transfer full); release it with fl_value_unref() or wrap it in
  // g_autoptr(FlValue).
  static FlValue* GetPluginConstantsForFirebaseApp(const firebase::App& app);
