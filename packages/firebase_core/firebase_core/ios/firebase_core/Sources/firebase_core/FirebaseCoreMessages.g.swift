...
private func doubleEqualsFirebaseCoreMessages(_ lhs: Double, _ rhs: Double) -> Bool {
...
func deepEqualsFirebaseCoreMessages(_ lhs: Any?, _ rhs: Any?) -> Bool {
  ...
  case (let lhsArray, let rhsArray) as ([Any?], [Any?]):
    ...
    for (index, element) in lhsArray.enumerated() {
      if !deepEqualsFirebaseCoreMessages(element, rhsArray[index]) {
        return false
      }
    }
    return true
  ...
  case (let lhsArray, let rhsArray) as ([Double], [Double]):
    guard lhsArray.count == rhsArray.count else { return false }
    for (index, element) in lhsArray.enumerated() {
      if !doubleEqualsFirebaseCoreMessages(element, rhsArray[index]) {
        return false
      }
    }
    return true
  ...
  case (let lhsDictionary, let rhsDictionary) as ([AnyHashable: Any?], [AnyHashable: Any?]):
    ...
    for (lhsKey, lhsValue) in lhsDictionary {
      ...
      for (rhsKey, rhsValue) in rhsDictionary {
        if deepEqualsFirebaseCoreMessages(lhsKey, rhsKey) {
          if deepEqualsFirebaseCoreMessages(lhsValue, rhsValue) {
            found = true
            break
          } else {
            return false
  ...
  case (let lhs as Double, let rhs as Double):
    return doubleEqualsFirebaseCoreMessages(lhs, rhs)
...
func deepHashFirebaseCoreMessages(value: Any?, hasher: inout Hasher) {
  let cleanValue = nilOrValue(value) as Any?
  if let cleanValue = cleanValue {
    if let doubleValue = cleanValue as? Double {
      doubleHashFirebaseCoreMessages(doubleValue, &hasher)
    } else if let valueList = cleanValue as? [Any?] {
      for item in valueList {
        deepHashFirebaseCoreMessages(value: item, hasher: &hasher)
      }
    } else if let valueList = cleanValue as? [Double] {
      for item in valueList {
        doubleHashFirebaseCoreMessages(item, &hasher)
      }
    } else if let valueDict = cleanValue as? [AnyHashable: Any?] {
      var result = 0
      for (key, value) in valueDict {
        var entryKeyHasher = Hasher()
        deepHashFirebaseCoreMessages(value: key, hasher: &entryKeyHasher)
        var entryValueHasher = Hasher()
        deepHashFirebaseCoreMessages(value: value, hasher: &entryValueHasher)
...
struct CoreFirebaseOptions: Hashable {
  ...
  func hash(into hasher: inout Hasher) {
    hasher.combine("CoreFirebaseOptions")
    deepHashFirebaseCoreMessages(value: apiKey, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: appId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: messagingSenderId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: projectId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: authDomain, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: databaseURL, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: storageBucket, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: measurementId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: trackingId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: deepLinkURLScheme, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: androidClientId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: iosClientId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: iosBundleId, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: appGroupId, hasher: &hasher)
  }
}
...
struct CoreInitializeResponse: Hashable {
  ...
  func hash(into hasher: inout Hasher) {
    hasher.combine("CoreInitializeResponse")
    deepHashFirebaseCoreMessages(value: name, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: options, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: isAutomaticDataCollectionEnabled, hasher: &hasher)
    deepHashFirebaseCoreMessages(value: pluginConstants, hasher: &hasher)
  }
}

private class FirebaseCoreMessagesPigeonCodecReader: FlutterStandardReader {
...
private class FirebaseCoreMessagesPigeonCodecWriter: FlutterStandardWriter {
...
private class FirebaseCoreMessagesPigeonCodecReaderWriter: FlutterStandardReaderWriter {
  override func reader(with data: Data) -> FlutterStandardReader {
    return FirebaseCoreMessagesPigeonCodecReader(data: data)
  }

  override func writer(with data: NSMutableData) -> FlutterStandardWriter {
    return FirebaseCoreMessagesPigeonCodecWriter(data: data)
  }
}
