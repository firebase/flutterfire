...
void SetUpFirebaseAppHostApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FirebaseAppHostApi> *api, NSString *messageChannelSuffix) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.firebase_core_platform_interface.FirebaseAppHostApi.registerLibrary", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:nullGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(registerLibraryLibrary:version:platform:completion:)], @"FirebaseAppHostApi api (%@) doesn't respond to @selector(registerLibraryLibrary:version:platform:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSString *arg_library = GetNullableObjectAtIndex(args, 0);
        NSString *arg_version = GetNullableObjectAtIndex(args, 1);
        NSString *arg_platform = GetNullableObjectAtIndex(args, 2);
        [api registerLibraryLibrary:arg_library version:arg_version platform:arg_platform completion:^(FlutterError *_Nullable error) {
          callback(wrapResult(nil, error));
        }];
      }];
    } else {
