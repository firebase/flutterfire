...
+ (nullable PigeonFirebaseOptions *)nullableFromMap:(NSDictionary *)dict { return (dict) ? [PigeonFirebaseOptions fromMap:dict] : nil; }
- (NSDictionary *)toMap {
  return @{
...
+ (instancetype)makeWithName:(NSString *)name
  ...
  PigeonInitializeReponse* pigeonResult = [[PigeonInitializeReponse alloc] init];
...
+ (PigeonInitializeReponse *)fromMap:(NSDictionary *)dict {
  ...
  pigeonResult.options = [PigeonFirebaseOptions nullableFromMap:GetNullableObject(dict, @"options")];
  NSAssert(pigeonResult.options != nil, @"");
  pigeonResult.isAutomaticDataCollectionEnabled = GetNullableObject(dict, @"isAutomaticDataCollectionEnabled");
  pigeonResult.pluginConstants = GetNullableObject(dict, @"pluginConstants");
  NSAssert(pigeonResult.pluginConstants != nil, @"");
  return pigeonResult;
}
+ (nullable PigeonInitializeReponse *)nullableFromMap:(NSDictionary *)dict { return (dict) ? [PigeonInitializeReponse fromMap:dict] : nil; }
- (NSDictionary *)toMap {
  return @{
...
- (void)writeValue:(id)value 
{
  ...
  if ([value isKindOfClass:[PigeonInitializeReponse class]]) {
    [self writeByte:129];
    [self writeValue:[value toMap]];
  } else 
{
...
void FirebaseCoreHostApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FirebaseCoreHostApi> *api) {
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        [api initializeAppAppName:arg_appName initializeAppRequest:arg_initializeAppRequest completion:^(PigeonInitializeReponse *_Nullable output, FlutterError *_Nullable error) {
  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        [api initializeCoreWithCompletion:^(NSArray<PigeonInitializeReponse *> *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    }
...
NSObject<FlutterMessageCodec> *FirebaseAppHostApiGetCodec() {
  ...
  static FlutterStandardMessageCodec *sSharedObject = nil;
  dispatch_once(&sPred, ^{
    FirebaseAppHostApiCodecReaderWriter *readerWriter = [[FirebaseAppHostApiCodecReaderWriter alloc] init];
    sSharedObject = [FlutterStandardMessageCodec codecWithReaderWriter:readerWriter];
  });
  return sSharedObject;
}
