...
private func doubleEqualsFirebaseRemoteConfigMessages(_ lhs: Double, _ rhs: Double) -> Bool {
  (lhs.isNaN && rhs.isNaN) || lhs == rhs
}

private func doubleHashFirebaseRemoteConfigMessages(_ value: Double, _ hasher: inout Hasher) {
  if value.isNaN {
...
func deepEqualsFirebaseRemoteConfigMessages(_ lhs: Any?, _ rhs: Any?) -> Bool {
  ...
  case let (lhsArray, rhsArray) as ([Any?], [Any?]):
    ...
    for (index, element) in lhsArray.enumerated() {
      if !deepEqualsFirebaseRemoteConfigMessages(element, rhsArray[index]) {
        return false
      }
    }
    return true
  ...
  case let (lhsArray, rhsArray) as ([Double], [Double]):
    guard lhsArray.count == rhsArray.count else { return false }
    for (index, element) in lhsArray.enumerated() {
      if !doubleEqualsFirebaseRemoteConfigMessages(element, rhsArray[index]) {
        return false
      }
    }
    return true
  ...
  case let (lhsDictionary, rhsDictionary) as ([AnyHashable: Any?], [AnyHashable: Any?]):
    ...
    for (lhsKey, lhsValue) in lhsDictionary {
      ...
      for (rhsKey, rhsValue) in rhsDictionary {
        if deepEqualsFirebaseRemoteConfigMessages(lhsKey, rhsKey) {
          if deepEqualsFirebaseRemoteConfigMessages(lhsValue, rhsValue) {
            found = true
            break
          } else {
            return false
...
func deepHashFirebaseRemoteConfigMessages(value: Any?, hasher: inout Hasher) {
  ...
  if let cleanValue {
    if let doubleValue = cleanValue as? Double {
      doubleHashFirebaseRemoteConfigMessages(doubleValue, &hasher)
      ...
      for item in valueList {
        doubleHashFirebaseRemoteConfigMessages(item, &hasher)
      }
    } else if let valueDict = cleanValue as? [AnyHashable: Any?] {
      var result = 0
      for (key, value) in valueDict {
        var entryKeyHasher = Hasher()
        deepHashFirebaseRemoteConfigMessages(value: key, hasher: &entryKeyHasher)
        var entryValueHasher = Hasher()
        deepHashFirebaseRemoteConfigMessages(value: value, hasher: &entryValueHasher)
...
struct RemoteConfigPigeonSettings: Hashable {
  ...
  func hash(into hasher: inout Hasher) {
    hasher.combine("RemoteConfigPigeonSettings")
    deepHashFirebaseRemoteConfigMessages(value: fetchTimeoutSeconds, hasher: &hasher)
    deepHashFirebaseRemoteConfigMessages(value: minimumFetchIntervalSeconds, hasher: &hasher)
  }
}
