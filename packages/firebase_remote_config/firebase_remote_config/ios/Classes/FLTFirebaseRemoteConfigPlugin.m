...
NSString *const kFirebaseRemoteConfigChannelName =
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  FLTFirebaseRemoteConfigPlugin *instance =
      [FLTFirebaseRemoteConfigPlugin sharedInstance];
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  FLTFirebaseMethodCallErrorBlock errorBlock = ^(
    ...
    if (code == nil) {
      details =
          [FLTFirebaseRemoteConfigUtils ErrorCodeAndMessageFromNSError:error];
      code = [details valueForKey:@"code"];
      message = [details valueForKey:@"message"];
    }
    if ([@"unknown" isEqualToString:code]) {
      NSLog(
          @"FLTFirebaseRemoteConfig: An error occurred while calling method %@",
          call.method);
    }
  ...
  if ([@"RemoteConfig#ensureInitialized" isEqualToString:call.method]) {
    [self ensureInitialized:call.arguments
    ...
    [self fetchAndActivate:call.arguments
        withMethodCallResult:methodCallResult];
  } else if ([@"RemoteConfig#setConfigSettings" isEqualToString:call.method]) {
    [self setConfigSettings:call.arguments
        withMethodCallResult:methodCallResult];
...
- (void)ensureInitialized:(id)arguments
     withMethodCallResult:(FLTFirebaseMethodCallResult *)result {
  FIRRemoteConfig *remoteConfig =
      [self getFIRRemoteConfigFromArguments:arguments];
  [remoteConfig
      ensureInitializedWithCompletionHandler:^(NSError *initializationError) {
...
- (void)getProperties:(id)arguments
    withMethodCallResult:(FLTFirebaseMethodCallResult *)result {
  FIRRemoteConfig *remoteConfig =
      [self getFIRRemoteConfigFromArguments:arguments];
  NSDictionary *configProperties =
      [self configPropertiesForInstance:remoteConfig];
...
- (void)setConfigSettings:(id)arguments
     withMethodCallResult:(FLTFirebaseMethodCallResult *)result {
  ...
  remoteConfigSettings.minimumFetchInterval =
      [minimumFetchInterval doubleValue];
  FIRRemoteConfig *remoteConfig =
      [self getFIRRemoteConfigFromArguments:arguments];
...
- (void)fetchAndActivate:(id)arguments
    withMethodCallResult:(FLTFirebaseMethodCallResult *)result {
  FIRRemoteConfig *remoteConfig =
      [self getFIRRemoteConfigFromArguments:arguments];
  [remoteConfig
...
- (FIRRemoteConfig *_Nullable)getFIRRemoteConfigFromArguments:
...
- (NSDictionary *)getAllParametersForInstance:(FIRRemoteConfig *)remoteConfig {
  ...
  [keySet
      addObjectsFromArray:[remoteConfig
                              allKeysFromSource:FIRRemoteConfigSourceStatic]];
  [keySet
      addObjectsFromArray:[remoteConfig
...
- (NSMutableDictionary *)createRemoteConfigValueDict:
    (FIRRemoteConfigValue *)remoteConfigValue {
  NSMutableDictionary *valueDict = [[NSMutableDictionary alloc] init];
  valueDict[@"value"] = [FlutterStandardTypedData
      typedDataWithBytes:[remoteConfigValue dataValue]];
...
- (NSDictionary *_Nonnull)pluginConstantsForFIRApp:(FIRApp *)firebase_app {
  FIRRemoteConfig *firebaseRemoteConfig =
      [FIRRemoteConfig remoteConfigWithApp:firebase_app];
  NSDictionary *configProperties =
      [self configPropertiesForInstance:firebaseRemoteConfig];
...
- (NSDictionary *_Nonnull)configPropertiesForInstance:
    (FIRRemoteConfig *)remoteConfig {
  ...
  NSNumber *lastFetchMillis =
      @([[remoteConfig lastFetchTime] timeIntervalSince1970] * 1000);

  NSMutableDictionary *configProperties = [[NSMutableDictionary alloc] init];
  [configProperties setValue:@([fetchTimeout longValue])
                      forKey:@"fetchTimeout"];
  [configProperties setValue:@([minimumFetchInterval longValue])
                      forKey:@"minimumFetchInterval"];
  [configProperties setValue:@([lastFetchMillis longValue])
                      forKey:@"lastFetchTime"];
  [configProperties
      setValue:[self mapLastFetchStatus:[remoteConfig lastFetchStatus]]
