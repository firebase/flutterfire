...
void main() {
  ...
  group('$MethodChannelCloudFirestore', () {
    ...
    MethodChannelCloudFirestore platform = MethodChannelCloudFirestore();
    ...
    setUp(() async {
      ...
      MethodChannelCloudFirestore.channel
          ...
          case 'Query#addSnapshotListener':
            ...
            Future<void>.delayed(Duration.zero).then<void>((_) {
              ...
              BinaryMessages.handlePlatformMessage(
                MethodChannelCloudFirestore.channel.name,
                MethodChannelCloudFirestore.channel.codec.encodeMethodCall(
    ...
    group('Transaction', () {
      test('runTransaction', () async {
        final Map<String, dynamic> result = await platform.runTransaction(
            appName,
            transactionHandler: (int id) async {},
      ...
      test('get', () async {
        await platform.getTransaction(appName,
            path: 'foo/bar', transactionId: 0);
      ...
      test('get notExists', () async {
        await platform.getTransaction(appName,
            path: 'foo/notExists', transactionId: 0);
      ...
      test('delete', () async {
        await platform.deleteTransaction(appName,
            path: 'foo/bar', transactionId: 0);
      ...
      group('CollectionsReference', () {
        test('listen', () async {
          final dynamic snapshot = await platform.getQuerySnapshots(
            appName,
            path: 'foo',
          ...
          final dynamic document = snapshot.documents[0];
      ...
      group('DocumentReference', () {
        test('listen', () async {
          final dynamic snapshot = await platform
        ...
        test('set', () async {
          await platform.setDocumentReferenceData(
            appName,
            path: 'foo/bar',
        ...
        test('update', () async {
          await platform.updateDocumentReferenceData(
            appName,
            path: 'foo/bar',
            data: <String, String>{'bazKey': 'quxValue'},
          );
        ...
        test('delete', () async {
          await platform.deleteDocumentReference(
            appName,
            path: 'foo/bar',
          );
          expect(
        ...
        test('get', () async {
          final dynamic snapshot = await platform.getDocumentReference(
            appName,
            path: 'foo/bar',
            source: 'cache',
          );
          ...
          final dynamic snapshot2 = await platform.getDocumentReference(
            appName,
            path: 'foo/notExists',
            source: 'default',
          );
          ...
          try {
            await platform.getDocumentReference(appName,
                path: 'baz', source: null);
