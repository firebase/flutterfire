...
- (void)setupSnapshotsInSyncListener:(id)arguments
  ...
  NSString *observerId = [[[NSUUID UUID] UUIDString] lowercaseString];

  NSString *channelName = [NSString
      stringWithFormat:@"%@/%@", kFLTFirebaseFirestoreSnapshotsInSyncEventChannelName, observerId];

  FlutterEventChannel *channel = [FlutterEventChannel eventChannelWithName:channelName
                                                           binaryMessenger:_binaryMessenger
                                                                     codec:_codec];

  [channel setStreamHandler:[[FLTSnapshotsInSyncStreamHandler alloc] init]];
  result.success(observerId);
}
...
- (void)setupDocumentReferenceSnapshotsListener:(id)arguments
  ...
  NSString *channelName = [NSString
      stringWithFormat:@"%@/%@", kFLTFirebaseFirestoreDocumentSnapshotEventChannelName, observerId];

  FlutterEventChannel *channel = [FlutterEventChannel eventChannelWithName:channelName
                                                           binaryMessenger:_binaryMessenger
                                                                     codec:_codec];

  [channel setStreamHandler:[[FLTDocumentSnapshotStreamHandler alloc] init]];
  result.success(observerId);
}
...
- (void)setupQuerySnapshotsListener:(id)arguments
  ...
  NSString *channelName = [NSString
      stringWithFormat:@"%@/%@", kFLTFirebaseFirestoreQuerySnapshotEventChannelName, observerId];

  FlutterEventChannel *channel = [FlutterEventChannel eventChannelWithName:channelName
                                                           binaryMessenger:_binaryMessenger
                                                                     codec:_codec];

  [channel setStreamHandler:[[FLTQuerySnapshotStreamHandler alloc] init]];
  result.success(observerId);
}
