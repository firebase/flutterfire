    ...
    if (api != nullptr) {
      channel->SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          api->QuerySnapshot(app_arg, path_arg, is_collection_group_arg, parameters_arg, options_arg, include_metadata_changes_arg, [reply](ErrorOr<std::string>&& output) {
            if (output.has_error()) {
              reply(WrapError(output.error()));
              return;
            }
    ...
    if (api != nullptr) {
      channel->SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          const auto& include_metadata_changes_arg = std::get<bool>(encodable_include_metadata_changes_arg);
          api->DocumentReferenceSnapshot(app_arg, parameters_arg, include_metadata_changes_arg, [reply](ErrorOr<std::string>&& output) {
            if (output.has_error()) {
              reply(WrapError(output.error()));
              return;
            }
