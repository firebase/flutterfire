...
const PigeonFirebaseSettings& PigeonFirebaseApp::settings() const {
  return settings_;
}

void PigeonFirebaseApp::set_settings(const PigeonFirebaseSettings& value_arg) {
  settings_ = value_arg;
}
...
void FirebaseFirestoreHostApi::SetUp(
  ...
  {
    auto channel = std::make_unique<BasicMessageChannel<>>(binary_messenger, "dev.flutter.pigeon.cloud_firestore_platform_interface.FirebaseFirestoreHostApi.aggregateQueryCount", &GetCodec());
    if (api != nullptr) {
      channel->SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          ...
          const auto& source_arg = std::any_cast<const AggregateSource&>(std::get<CustomEncodableValue>(encodable_source_arg));
          api->AggregateQueryCount(app_arg, path_arg, parameters_arg, source_arg, [reply](ErrorOr<double>&& output) {
            if (output.has_error()) {
              reply(WrapError(output.error()));
              return;
            }
