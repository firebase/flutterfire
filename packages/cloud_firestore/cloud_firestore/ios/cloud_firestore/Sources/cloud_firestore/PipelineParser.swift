...
/// `BridgeWrapper.bridge` is internal to FirebaseFirestore, so typed expressions are lowered
/// through public `.bridge` members where available and Mirror otherwise.
    ...
    for i in 0..<stages.count {
      ...
      if i == 0 {
        ...
        if stageName == "where" {
          ...
          let otherPipeline = try buildPipeline(firestore: firestore, stages: nestedStages)
