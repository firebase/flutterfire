...
+ (NSArray *)ErrorCodeAndMessageFromNSError:(NSError *)error {
  ...
  case FIRFirestoreErrorCodeAborted:
    code = @"aborted";
    message = @"The operation was aborted, typically due to a concurrency "
              @"issue like transaction "
              @"aborts, etc.";
    break;
  ...
  case FIRFirestoreErrorCodeDeadlineExceeded:
    ...
    message = @"Deadline expired before operation could complete. For "
              @"operations that change the "
              @"state of the system, this error may be returned even if the "
              @"operation has "
  ...
  case FIRFirestoreErrorCodeFailedPrecondition:
    ...
    if ([error.localizedDescription
            containsString:@"query requires an index"] ||
        [error.localizedDescription
            containsString:@"requires a COLLECTION_GROUP_DESC index"]) {
      message = error.localizedDescription;
    } else {
      message = @"Operation was rejected because the system is not in a state "
                @"required for the "
                @"operation's execution. If performing a query, ensure it has "
                @"been indexed via "
  ...
  case FIRFirestoreErrorCodeResourceExhausted:
    ...
    message = @"Some resource has been exhausted, perhaps a per-user quota, or "
              @"perhaps the "
              @"entire file system is out of space.";
    break;
  case FIRFirestoreErrorCodeUnauthenticated:
    code = @"unauthenticated";
    message = @"The request does not have valid authentication credentials for "
              @"the operation.";
    break;
  case FIRFirestoreErrorCodeUnavailable:
    code = @"unavailable";
    message = @"The service is currently unavailable. This is a most likely a "
              @"transient "
