...
class ResultTreeProcessor {
  ...
  Future<DehydrationResult> dehydrate(String queryId, Map<String, dynamic> serverResponse,
      CacheProvider cacheProvider) async {
    ...
    final rootNode = _dehydrateNode(
        queryId, jsonData, cacheProvider, impactedQueryIds);
        
        //debug only    
        
        print("dehydrated rootNode ${jsonEncode(rootNode.toJson(mode: EncodingMode.dehydrated))}");
    
    return DehydrationResult(rootNode, impactedQueryIds);
  }

  EntityNode _dehydrateNode(String queryId, dynamic data, CacheProvider cacheProvider,
    ...
    if (data is Map<String, dynamic>) {
      if (data.containsKey(GlobalIDKey)) {
        ...
        for (final entry in data.entries) {
          ...
          if (value is Map<String, dynamic>) {
            EntityNode en =  _dehydrateNode(
                queryId, value, cacheProvider, impactedQueryIds);
            nestedObjects[key] = en;
            if (en != null) {
            print('dehydrate - got nestedObject EN for key ${key} ${en!.scalarValues?.length} ${en!.nestedObjectLists?.length} ${en!.nestedObjects?.length}');
            } else {
        ...
        print("dehydrate - returning EN ${existingEdo.guid} with EDO ${existingEdo}");
        return EntityNode(
            entity: existingEdo,
      ...
      print("dehydrate - no globalID ${data}");
        ...
        for (final entry in data.entries) {
          ...
          if (value is Map<String, dynamic>) {
            nestedObjects[key] =  _dehydrateNode(
                queryId, value, cacheProvider, impactedQueryIds);
          } else if (value is List) {
            ...
            nestedObjectLists[key] = nodeList;
            print("dehydrate - added to lists ${key} ${nodeList.length} ${jsonEncode(nodeList)}");
          } else {
            scalarValues[key] = value;
          }
        }
        print("dehydrate - returning an EN with scalaraValues ${scalarValues.length} nestedObjectLists ${nestedObjectLists.length} nestedObjects ${nestedObjects.length}");
      ...
      throw DataConnectError(DataConnectErrorCode.codecFailed, 'Unexpected object type while caching');
    }
     ...
     /*else if (data is List) {
      ...
      for (final item in data) {
        nodeList.add(
           _dehydrateNode(queryId, item, cacheProvider, impactedQueryIds));
      }
      return EntityNode(nestedObjectLists: {'list': nodeList});
    } else {
      return EntityNode(scalarValues: {'value': data});
    }*/
  }
  ...
  Future<Map<String, dynamic>> hydrate(
    ...
    return await _hydrateNode(dehydratedTree, cacheProvider) as Map<String, dynamic>;
  }
