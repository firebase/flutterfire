...
class PigeonListOptions {
 public:
  ...
  // Constructs an object setting all fields.
  explicit PigeonListOptions(const int64_t* max_results,
                             const std::string* page_token);

  // If set, limits the total number of `prefixes` and `items` to return.
  //
  // The default and maximum maxResults is 1000.
  const int64_t* max_results() const;
  void set_max_results(const int64_t* value_arg);
 ...
 private:
  ...
  friend class FirebaseStorageHostApi;
  friend class FirebaseStorageHostApiCodecSerializer;
  std::optional<int64_t> max_results_;
