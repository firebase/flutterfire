...
- (void)onDeepLinkResult:(FIRDynamicLink *_Nullable)dynamicLink error:(NSError *_Nullable)error {
  if (_initiated) {
    ...
    } else if (dynamicLink.url != nil || _initialLink == nil) { // We'd like to overwrite initial link only if it's the first time or if we overwrite it with url that is not nil
