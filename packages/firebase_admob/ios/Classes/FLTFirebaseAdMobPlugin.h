 ...
 *
 * When a Native Ad is created in Dart, this protocol is responsible for building the `GADUnifiedNativeAdView`. Register a class that
 * implements this with a `FLTFirebaseAdMobPlugin` to use in conjunction with Flutter.
 */
 ...
 * @param customOptions Used to pass additional custom options to create the `GADUnifiedNativeAdView`. Nullable.
...
/**
 * Adds a `FLTNativeAdFactory` used to create a `GADUnifiedNativeAdView`s from a Native Ad created in Dart.
 *
...
/**
 * Unregisters a `FLTNativeAdFactory` used to create `GADUnifiedNativeAdView`s from a Native Ad created in Dart.
 *
 * @param registry maintains access to a `FLTFirebaseAdMobPlugin `instance.
 * @param factoryId a unique identifier for the ad factory. The Native Ad created in Dart includes
 *     a parameter that refers to this.
 * @return the previous `FLTNativeAdFactory` associated with this factoryId, or null if there was none for this factoryId.
 */
+ (id<FLTNativeAdFactory>)registerNativeAdFactory:(NSObject<FlutterPluginRegistry> *)registry
                                        factoryId:(NSString *)factoryId;
@end
