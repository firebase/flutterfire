...
data class InternalAppCheckTokenResult (
  val token: String,
...
)
 {
  ...
  override fun hashCode(): Int {
    ...
    result = 31 * result + GeneratedAndroidFirebaseAppCheckPigeonUtils.deepHash(this.expirationTimestamp)
    return result
  }
}
    ...
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          InternalAppCheckTokenResult.fromList(it)
        }
      }
...
interface FirebaseAppCheckHostApi {
  fun activate(appName: String, androidProvider: String?, appleProvider: String?, debugToken: String?, recaptchaSiteKey: String?, callback: (Result<Unit>) -> Unit)
  fun getToken(appName: String, forceRefresh: Boolean, callback: (Result<String?>) -> Unit)
  fun getTokenResult(appName: String, forceRefresh: Boolean, callback: (Result<InternalAppCheckTokenResult?>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: FirebaseAppCheckHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            api.activate(appNameArg, androidProviderArg, appleProviderArg, debugTokenArg, recaptchaSiteKeyArg) { result: Result<Unit> ->
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.firebase_app_check_platform_interface.FirebaseAppCheckHostApi.getTokenResult$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val appNameArg = args[0] as String
            val forceRefreshArg = args[1] as Boolean
            api.getTokenResult(appNameArg, forceRefreshArg) { result: Result<InternalAppCheckTokenResult?> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(GeneratedAndroidFirebaseAppCheckPigeonUtils.wrapError(error))
              } else {
