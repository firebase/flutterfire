...
Before using the RealTime Database, you must first have ensured you have [initialized FlutterFire](../overview.mdx#initializing-flutterfire).
...
Important to keep in mind - "Setting applyLocally to false doesn't fundamentally change how transactions behave on the wire. It only changes when the client/SDK raises events."
