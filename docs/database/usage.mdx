...
TransactionResult result = await ref.runTransaction((Object? post) {
  ...
  Map<String, dynamic> _post = post as Map<String, dynamic>;
...
```dart
await ref.runTransaction((Object? post, applyLocally: false) {
 // ...
});
```
...
try {
  TransactionResult result = await ref.runTransaction((Object? user) {
    if (user !== null) {
      throw AbortTransactionException('User already exists!');
    }

    // ...
  });
} on FirebaseException catch (e) {
  print(e.code); // abort-transaction
  print(e.message); // User already exists!
}
...
// If the user goes offline, set the user status to `false`:
