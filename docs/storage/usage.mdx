...
### Cross-origin access (CORS for web only)

If you wish to download a file on the `web` platform, you must configure your Cloud Storage bucket for cross-origin access (CORS).
Please follow the [Firebase Storage documentation](https://firebase.google.com/docs/storage/web/download-files#cors_configuration) for the latest
method for configuring your project to handle CORS.
...
If you are using the local [Storage emulator](https://firebase.google.com/docs/rules/emulator-setup), then it is possible to
connect to this by passing a `host` & `port` to the `useStorageEmulator` method immediately after initializing Firebase.
