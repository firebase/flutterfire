...
To install FlutterFire UI, ensure you have first [installed & initialized FlutteFire](https://firebase.flutter.dev/docs/overview) by following the 
documentation.

Next, install the `firebase_ui` package by running the following command in your terminal:

```bash
flutter pub add firebase_ui
```

**Note; you must install any FlutterFire plugins such as `firebase_auth` before you can use the Auth UI widgets and utilities.**
