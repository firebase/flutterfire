...
It takes care of creating an instance of an `AuthFlow` with a provided `ProviderConfiguration`,
...
Even if you want to have a full controll over the look and feel of every single widget,
...
Let's have a look on how to use `AuthFlowBuilder` in your custom widgets.
...
Iniital state is `AwaitingEmailAndPassword`.
