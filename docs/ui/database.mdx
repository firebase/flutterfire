---
title: FlutterFire UI for Realtime Database
...
[installed on your project](https://firebase.flutter.dev/docs/database/overview#installation).
...
dataset, as it enables the application to render faster as well as reducing network
...
using the Realtime Database database with the `FirebaseListView` widget.
...
```dart
final usersQuery = FirebaseDatabase.instance.collection('users').orderBy('name');
...
The `FirebaseListView` widget is built on-top of Flutter's own [`ListView`](https://api.flutter.dev/flutter/widgets/ListView-class.html)
...
(such as permission denied). You can override this behaviour by providing a `loadingBuilder` and `errorBuilder` parameters to the widget:
...
In many cases, the `FirebaseListView` widget is enough to render simple lists of collection data.
However, you may have specific requirements which require more control over the widget's behaviour
(such as using a [`GridView`](https://api.flutter.dev/flutter/widgets/GridView-class.html)).

The `FirebaseQueryBuilder` provides the building blocks for advanced configuration as the expense of
...
Much like the `FirebaseListView` widget, provide a query and builder:
...
The main differences to note here is that the `builder` property returns a `FirebaseQueryBuilderSnapshot`, rather
...
1. The `FirebaseQueryBuilder` does not provide a list-view based handler, instead you must provide your own implementation.
