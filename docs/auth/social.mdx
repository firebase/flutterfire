...
You will need to follow the steps in the plugin [`documentation`](https://pub.dev/packages/flutter_facebook_auth) to ensure that both the Android & iOS Facebook SDKs have been initialized correctly. Once complete, trigger the sign-in flow, create a Facebook credential and sign the user in:
