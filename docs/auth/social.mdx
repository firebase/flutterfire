...
:::caution
Dart-only initialization isn't yet support by `google_sign_in` plugin, in which case you still have to do the [manual installation](./manual-installation.mdx) if you're using Google as an auth provider.
:::
