...
For iOS, you need to set one place manually.
Open Xcode and in GoogleService-Info.plist, add REVERSED_CLIENT_ID to the section called URL Schemes.
See the link below for detailed instructions.

- [`Firebase > Docs > Guides > Authenticate Using Google Sign-In on iOS - 2. Implement Google Sign-In`](https://firebase.google.com/docs/auth/ios/google-signin#2_implement_google_sign-in)
- [`GitHub: googlesamples/google-services/issues/81`](https://github.com/googlesamples/google-services/issues/81#issuecomment-271095633)
