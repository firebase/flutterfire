...
Once called, the stream provides an immediate event of the users current authentication state, and then provides subsequent events whenever
...
As mentioned above, some operations such as deleting the user, updating their email address or provider require the user to have recently
