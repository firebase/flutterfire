...
Or, if you need all user change events which include auth state changes and id token changes along with any other user updates, you can subscribe via the
`userChanges()` method instead. This stream provides realtime updates to the `User` class without having to call `reload()`, such as when credentials are linked,
...
On native platforms such as Android & iOS, this behaviour is not configurable and the user's authenticate state will be persisted on-device
