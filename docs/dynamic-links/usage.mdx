...
A dynamic link can be created directly from the Firebase Console, or programatically via the `firebase_dynamic_links` plugin. Once a link has been created,
...
To handle an Dynamic Link in your application, two scenarios require implementing.
...
This is an asynchronous request, so it make sense to handle a link before rendering application logic, such as 
...
getter returns a `Stream` containing a nullable `PendingDynamicLinkData`:
