...
The Object Document Model or ODM is an interface that treats a document as a tree structure wherein each node is an object representing a part of the document. . 
Each branch of the tree ends in a node, and each node contains objects. 
ODM methods allow programmatic access to the tree; with them one can change the structure, style or content of a document. 
...
Before getting started, ensure you have [initialized FlutterFire](../overview.mdx).
...
The ODM depends on the `cloud_firestore` plugin, so first ensure you have it [installed on your project](../firestore/overview.mdx#installation).
