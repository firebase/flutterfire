...
The above code will download the latest available `myModel` modle to the device, since the `DownloadType.latestModel` has been specified.
...
You can also provide specific conditions for the download, such as whether the model shoul dbe download whilst the device has access to
