...
```dart
AppCheckTokenResult result = await FirebaseAppCheck.instance.getToken();
print(result.token);
```
...
```dart
AppCheckTokenResult result = await FirebaseAppCheck.instance.getToken(true);
```

## Listen to token changes

To subscribe to App Check token changes, call the `onTokenChange` method:

```dart
FirebaseAppCheck.instance.tokenChanges().listen((result) {
  print(result.token);
});
```
