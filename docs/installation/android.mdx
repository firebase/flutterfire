...
Due to the large number of classes in some FlutterFire plugins, it may bump you over the 64k method limit on the Android
 build system and you will get an error stating `Cannot fit requested classes in a single dex file`.

If do get this error, we suggest enabling Multidex for Android.
