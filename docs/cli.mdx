...
Additionally, if you use the [`google_sign_in` provider](./auth/social#google) for Authentication in `firebase_auth` then this also requires the Google services JSON [manual installation](./manual-installation.mdx) setup on Android.
