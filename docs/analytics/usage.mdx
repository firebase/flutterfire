...
You can log custom events by calling the `logEvent` method. The method accepts a event name (which will
...
| `logAddToWishlist`        | This event signifies that an item was added to a wishlist. Use this event to identify popular gift items in your app.                           |
| `logAdImpression`         | This event signifies that an item was added to a wishlist. Use this event to identify popular gift items in your app.                           |
...
To help identify the areas of your application which users spend time, you can call the `setCurrentScreen` method:
...
If you have common parameters which need to be sent with each event, call the `setDefaultEventParameters` method:
...
You can set the current analytics session to assign events to a user, by ID with the `setUserId` method:
