...
As explained in the [Usage documentation](usage.mdx), permission must be requested from your iOS users in order to display
remote notifications from FCM, via the [`requestNotificationPermissions`](!firebase_messaging.FirebaseMessaging.requestNotificationPermissions) API:

```dart
FirebaseMessaging messaging = FirebaseMessaging.instance;

bool isPermissionGiven = await messaging.requestNotificationPermissions();
