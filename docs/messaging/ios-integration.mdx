...
On Apple devices, in order for incoming [Notifications](notifications.mdx) to display images, you must add an additional notification
service extension.
...
Your device can now display images in a notification! Bear in mind, a 300KB max image size is enforced by the device.
