...
In the `web/index.html` file, add a `script` tag which adds the version to the window object:
...
If you wish to exclude scripts from automatically being injected, provide an array of services in
...
<html>
<head>
  <script>window.flutterfire_ignore_scripts = ['analytics'];</script>
</head>
...
```

If you ignore any services from being injected, you must ensure you manually inject them before any
usage of the plugin.
