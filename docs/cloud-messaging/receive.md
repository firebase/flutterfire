...
operating system settings. On Android >= 13 versions, there is no way to determine if the user has chosen whether to grant/deny permission. A `denied` value conveys an undetermined/denied permission state, and it will be up to you to track if a permission request has been made.
