...
On iOS, macOS, web and android (android =>13. Lower versions do not require permission), before FCM payloads can be received on your device,
...
Note: On Android <=12 `authorizationStatus` will return `authorized` if the user has not disabled notifications for the app via the operating systems settings.
