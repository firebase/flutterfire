...
To access the default app, call the [`initializeApp`](!firebase_core.FirebaseApp.initializeApp) or [`app`](!firebase_core.FirebaseApp.app) method on [`FirebaseCore`](!firebase_core):
...
To initialize a secondary app, call the [`initializeApp`](!firebase_core.FirebaseApp.initializeApp) method with a name and options:
...
It is also possible to fetch all existing apps at once via the [`apps`](!firebase_core.FirebaseApp.apps) property on [`FirebaseCore`](!firebase_core):
...
The convenient way to use the default app is by accessing the [`instance`](!firebase_core.FirebaseApp.instance) variable on each plugin base class. For example
...
If instead you'd like to use a secondary app, pass it to the [`instanceFor`](!firebase_core.FirebaseApp.instanceFor) static method on each plugin base class. For
