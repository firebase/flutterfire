...
Let's have a look at calling Cloud functions with additional Parameters. For example:
...
Future<void> writeMessage(String message) async {
    ...
    final resp = await callable.call(<String, dynamic>{
      'text': 'A message send from a client device',
    });
...
What if we want to change things after data has been uploaded to the cloud? Here we can listen to changes of documents such as `onCreate`, `onUpdate`,`onDelete`, `onWrite`. In the next example we are listening for new messages in a firestore directory and making a copy as uppercase:

```javascript title="Deployed Function"
// Listens for new messages added to /messages/:documentId/original and creates an
// uppercase version of the message to /messages/:documentId/uppercase
exports.makeUppercase = functions.region('europe-west3').firestore.document('/messages/{documentId}')
