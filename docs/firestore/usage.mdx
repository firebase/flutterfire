...
- And throughout https://firebase.google.com/docs/firestore/... Most pages that
  have code snippets have Flutter examples. If you find a page that's missing
  Flutter snippets, please [file a bug](https://github.com/firebase/flutterfire/issues/new/choose).
...
By default, Firestore references manipulate a `Map<String, dynamic>` object. The downside is that we lose type safety.
One solution is to use `withConverter`, which will modify methods like `CollectionReference.add` or `Query.where` to be type-safe.
...
Calling [`set`](!cloud_firestore.DocumentReference.set) with an id that already exists on the collection will replace all the document data.  You can also specify `SetOptions(merge: true))` on the query, and this will merge the existing document with the data passed into the `set()`:
...
you want to write multiple documents without using the documents current state, a [batch write](#batch-write)
should be used.
...
connect to this using the `useFirestoreEmulator` method. Ensure you pass the correct port on which the Firebase emulator is running on.
...
If you have [setup a data bundle](https://firebase.google.com/docs/firestore/bundles#building_data_bundles_on_the_server) and are
[currently serving it](https://firebase.google.com/docs/firestore/bundles#serving_data_bundles), you may load data bundles
into your app.
...
If you have loaded a bundle with a [named query](https://firebase.google.com/docs/firestore/bundles#building_data_bundles_on_the_server), you may retrieve the
query snapshot by calling the `namedQueryGet()` API:
...
To support more frequent counter updates, create a distributed counter. Each counter is a document with a subcollection of `shards`,
...
:::note
An alternative solution is to use **Firebase extensions**, 
please refer to [the Ditributed Counters extension and how to set it up here](https://firebase.google.com/products/extensions/firebase-firestore-counter).
:::
...
Refer to [the official documentation on distributed counters](https://firebase.google.com/docs/firestore/solutions/counters) for more details.