...
Calling [`set`](!cloud_firestore.DocumentReference.set) with an id that already exists on the collection will replace all the document data, unless you specify that the data should be merged into the existing document, as follows:
