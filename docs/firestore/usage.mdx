...
```dart
final moviesRef = Firestore.instance.collection('movies').withConverter(
...
Calling [`set`](!cloud_firestore.DocumentReference.set) with an id that already exists on the collection will replace all the document data.
...
QuerySnapshot<Map<String, Object?>>  snapshot = await firestore.namedQueryGet('sports-teams', options: const GetOptions(source: Source.cache));
