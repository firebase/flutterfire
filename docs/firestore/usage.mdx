...
Stream documentStream = Firestore.instance.collection('users').document('ABC123').snapshots();
```
...
To access the documents within a [`QuerySnapshot`](!cloud_firestore.QuerySnapshot), call the [`documents`](!cloud_firestore.QuerySnapshot.documents) property,
...
unique auto-generated ID. If you'd like to specify your own ID, call the [`setData`](!cloud_firestore.DocumentReference.setData)
...
Calling [`setData`](!cloud_firestore.DocumentReference.setData) with a documentId that already exists, it will replace all the document data

### Updating documents

Sometimes you may wish to update a document, rather than replacing all of the data. The [`setData`](!cloud_firestore.DocumentReference.setData)
method above replaces any existing data on a given [`DocumentReference`](!cloud_firestore.DocumentReference).
If you'd like to update a document instead, use the [`updateData`](!cloud_firestore.DocumentReference.updateData) method:
...
combination of [`setData`](!cloud_firestore.WriteBatch.setData), [`updateData`](!cloud_firestore.WriteBatch.updateData),
